...
base3 = AutoModelForCausalLM.from_pretrained(BASE_MODEL, torch_dtype=torch.float16, device_map="auto")
model_fast2 = PeftModel.from_pretrained(base3, FAST10_LORA)
model_fast2.eval()
...
{{"prediction":"Attack"}} or {{"prediction":"Normal"}}"""
    ...
    inputs = tokenizer(prompt_text, return_tensors="pt").to(model_fast2.device)

    with torch.no_grad():
        outputs = model_fast2.generate(**inputs, max_new_tokens=20, do_sample=False,
...
slow_test_files = ['run_099.csv', 'run_100.csv']

print("=" * 60)
print("FAST DRIFT MODEL → SLOW DRIFT TEST DATA (cross-test)")
print("=" * 60)

for fname in slow_test_files:
    df = pd.read_csv(os.path.join(MINICPS_PATH, fname))
    ...
    for i in range(window_size, len(df), window_size):
        ...
        results_cross2.append(pred)
        labels_cross2.append(label)

TP = sum(1 for t,p in zip(labels_cross2,results_cross2) if t=='Attack' and p=='Attack')
TN = sum(1 for t,p in zip(labels_cross2,results_cross2) if t=='Normal' and p=='Normal')
FP = sum(1 for t,p in zip(labels_cross2,results_cross2) if t=='Normal' and p=='Attack')
FN = sum(1 for t,p in zip(labels_cross2,results_cross2) if t=='Attack' and p=='Normal')
