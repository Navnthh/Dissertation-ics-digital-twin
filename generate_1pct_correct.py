...
def plot_1pct_correct(attack_file, cascade_file, normal_file, output_name):
    ...
    fig.suptitle('SENSOR1 1% Range Drift Attack — All Sensors (%)\nNormal vs During Attack',
    ...
    # SENSOR1 — real vs fake in percentage
    ax1 = axes[0][0]
    ...
    ax1.plot(attack_df['cycle'].values, real_pct,
             color='#4db8ff', linewidth=2,
             label='Real value %', alpha=0.9)
    ax1.plot(attack_df['cycle'].values, fake_pct,
    ...
    ax1.set_ylim(-10, 110)
    ax1.set_title('SENSOR1 — Tank Level (%) — ATTACKED',
    ...
    # SENSOR3 — normal vs during attack
    ax3 = axes[1][0]
    min_v, max_v = ranges['SENSOR3']
    normal_pct = to_percent(normal_df['SENSOR3'].values, min_v, max_v)
    attack_pct = to_percent(cascade_df['SENSOR3'].values, min_v, max_v)
    ax3.plot(normal_df['cycle'].values, normal_pct,
             color='#ffd94d', linewidth=2,
             label='Normal %', alpha=0.9)
    ax3.plot(cascade_df['cycle'].values, attack_pct,
             color='#f85149', linewidth=2,
             label='During attack %', linestyle='--', alpha=0.9)
    ...
    ax3.axhline(y=0, color='orange', linewidth=1,
               linestyle=':', alpha=0.7, label='Lower 0%')
    ax3.set_ylim(-10, 110)
    ax3.set_title('SENSOR3 — Bottle Level (%)',
                 fontsize=11, fontweight='bold')
...
plot_1pct_correct(
    os.path.join(LOG_DIR, 'slow_refill_1pct_SENSOR1_20260519_024751.csv'),
    os.path.join(LOG_DIR, 'cascade_v3_SENSOR1_1pct_anystart_20260519_024804.csv'),
    os.path.join(LOG_DIR, 'cascade_v3_NO_ATTACK_5min_20260507_040228.csv'),
    '45_SENSOR1_1pct_anystart_percent'
)
print('Done!')
